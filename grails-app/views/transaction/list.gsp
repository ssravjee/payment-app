			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="creditor" title="${message(code: 'transaction.creditor.label', default: 'Creditor')}" />
					
						<g:sortableColumn property="debtor" title="${message(code: 'transaction.debtor.label', default: 'Debtor')}" />
				...
				<g:each in="${transactionInstanceList}" status="i" var="transactionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: transactionInstance, field: "creditor")}</td>
					
						<td>${fieldValue(bean: transactionInstance, field: "debtor")}</td>
